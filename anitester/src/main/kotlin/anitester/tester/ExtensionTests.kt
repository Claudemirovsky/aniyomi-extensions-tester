package anitester.tester

import anitester.cmd.RED
import anitester.cmd.RESET
import anitester.cmd.dto.ConfigsDto
import anitester.cmd.printAnime
import anitester.cmd.printEpisode
import anitester.cmd.printLine
import anitester.cmd.printTitle
import anitester.cmd.printVideo
import anitester.cmd.timeTest
import anitester.tester.models.ResultDto
import anitester.tester.models.TestsEnum
import anitester.tester.models.TestsResultsDto
...
class ExtensionTests(
    ...
    private val ANIDETAILS_URL: String = configs.animeUrl
    ...
    private suspend fun testAnimeDetails() {
        ...
        writeTestSuccess(TestsEnum.ANIDETAILS) {
            json.encodeToJsonElement(details as SAnimeImpl).jsonObject
        }
    }
    ...
    private suspend fun testEpisodeList() {
        ...
        writeTestSuccess(TestsEnum.EPLIST) {
            json.encodeToJsonElement(episodeList.map { it as SEpisodeImpl })
                .jsonArray
        }
    }
    ...
    private suspend fun testVideoList() {
        ...
        if (configs.checkVideos) {
            videoList.parallelForEach { video ->
        ...
        videoList.forEach(::printItemOrJson)

        writeTestSuccess(TestsEnum.VIDEOLIST) {
            json.encodeToJsonElement(videoList.map(::VideoDto)).jsonArray
        }
    }
     ...
     * @param resultFetcher An lambda block that will return the data generated
     * by the test. It wil run ONLY if `configs.completeResults` is set (--complete-results -C).
     */
    ...
    private suspend inline fun <T> Iterable<T>.parallelForEach(crossinline f: suspend (T) -> Unit): Unit =
