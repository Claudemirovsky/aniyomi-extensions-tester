...
class CustomDriver : Driver() {
    ...
    init {
        val nodeManager = NodejsManager()
        nodeManager.getCacheDir().let {
        ...
        val nodePath = if (System.getProperty(PLAYWRIGHT_NODEJS_FOUND) != null) {
            ...
            val newPath = nodeManager.getNodejsPath(env)
            newPath?.let {
    ...
    private fun installBrowser(env: Map<String, String>) {
        ...
        if (!result) {
            process.destroy()
            throw RuntimeException("Timed out waiting for firefox to install")
        }
