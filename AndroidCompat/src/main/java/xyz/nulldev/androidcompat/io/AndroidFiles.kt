...
class AndroidFiles(val configManager: ConfigManager = GlobalConfigManager) {
    ...
    val externalFilesDirs: List<File> get() = filesConfig.externalFilesDirs.map(::registerFile)
    val obbDirs: List<File> get() = filesConfig.obbDirs.map(::registerFile)
    val cacheDir: File get() = registerFile(filesConfig.cacheDir)
    val codeCacheDir: File get() = registerFile(filesConfig.codeCacheDir)
    val externalCacheDirs: List<File> get() = filesConfig.externalCacheDirs.map(::registerFile)
    val externalMediaDirs: List<File> get() = filesConfig.externalMediaDirs.map(::registerFile)
